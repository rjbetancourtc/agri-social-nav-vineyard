...
public class HumanKinematics : MonoBehaviour
{
    ...
    /// <summary>
    /// Reinicia el estado interno del estimador cinemático.
    ///
    /// Debe ejecutarse después de restaurar el Transform
    /// del humano y antes de comenzar una nueva corrida.
    /// </summary>
    public void ResetKinematicsState()
    {
        // La posición anterior debe coincidir con
        // la posición inicial restaurada.
        previousPosition = transform.position;

        // La corrida comienza sin velocidad heredada.
        previousVelocity = Vector3.zero;

        // Limpiar las salidas cinemáticas.
        PlanarVelocity = Vector3.zero;
        PlanarAcceleration = Vector3.zero;

        // Reconstruir heading a partir de la orientación
        // restaurada del humano.
        Vector3 fwd = transform.forward;
        fwd.y = 0f;
        ...
        HeadingRad = Mathf.Atan2(
            fwd.z,
            fwd.x
        );

        Debug.Log(
            "[HumanKinematics] Reset completado | " +
            "Human = " + gameObject.name +
            " | Position = " + previousPosition +
            " | Velocity = " + PlanarVelocity +
            " | Acceleration = " + PlanarAcceleration +
