...
public class RobotSocialNavController : MonoBehaviour
{
    ...
    [Header("Modo experimental M0-M4")]
    ...
    public float switchDistance = 0.35f;
...
private void UpdateM0_NavMeshOnly()
{
    ...
    lastFinalDirection = (globalTarget - transform.position).normalized;
}
...
private void UpdateM3_IsotropicProxemics()
{
    ...
    Vector3 finalDir = (globalDir + avoidDir).normalized;
...
private void UpdateM4_FullAnisotropicHysteresis()
{
    ...
    {
        if (pointA_R == null || pointB_R == null) return;
        if (patrolTarget == null) patrolTarget = pointB_R;
    ...
    private Vector3 GetCurrentGlobalTarget()
    {
        ...
        {
            if (patrolTarget != null) return patrolTarget.position;
    ...
    {
        if (kin != null) return kin.HeadingRad;
    ...
    {
        if (nearestHuman == null) return Vector3.zero;
        if (nearestHumanDistance > avoidanceInfluenceDistance) return Vector3.zero;
        ...
        if (nearestHumanDistance <= personalZone) gain *= 2.0f;
        if (nearestHumanDistance <= intimateZone) gain *= 4.0f;
    ...
    private Vector3 ComputeFinalDirection(Vector3 globalDir, Vector3 avoidDir)
    {
        ...
        switch (mode)
        {
            ...
            case SocialMode.Auto:
                finalDir = (globalDir + avoidDir).normalized;
                break;
        }
